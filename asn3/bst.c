...
int bst_node_check(struct bst_node* node) {
  if (node==NULL)
    return(0);
  return(bst_node_check(node->left) + 1 + bst_node_check(node->right));
}

int bst_size(struct bst* bst) {
  assert(bst);
  return(bst_node_check(bst->root));
}
 ...
 */
int check_node_height(struct bst_node* node) {
  if (node==NULL)
    return(0);
  int leftHeight = check_node_height(node->left);
  int rightHeight = check_node_height(node->right);
...
int bst_height(struct bst* bst) {
  assert(bst);
  return(check_node_height(bst->root));
}
 ...
 */
 ...
 int check_paths(int target, struct bst_node* node) {
   //reset counter
   if (node==NULL)
    return (target==0);
  int x = 0;
  //remove
  int check = target-node->val;
  //return 1 if leaf and path is correct
  if ((check==0) && (node->left==NULL) && (node->right==NULL))
    return(1);
  //make x = 1 if it is already, or if the next path is correct
  if (node->left)
    x = x || check_paths(check,node->left);
  if (node->right)
    x = x || check_paths(check,node->right);
  return(x);
 }

int bst_path_sum(int sum, struct bst* bst) {
  assert(bst);
  return(check_paths(sum,bst->root));
}
 ...
 */
 ...
 void moveItems(struct stack* from, struct stack* to) {
   while(!stack_isempty(from))
    stack_push(to,stack_pop(from));
 }

 void inOrder(struct stack* s,struct stack* temp,struct bst_node* node) {
    if (node == NULL)
           return;
      inOrder(s,temp,node->left);
      moveItems(s,temp);
      stack_push(s,node->val);
      moveItems(temp,s);
      inOrder(s,temp,node->right);
 }
...
struct bst_iterator* bst_iterator_create(struct bst* bst) {
  ...
  i->s = malloc(sizeof(struct stack*));
  i->temp = malloc(sizeof(struct stack*));
  i->node = malloc(sizeof(struct node*));
  inOrder(i->s,i->temp,bst->root);
  return(i);
}
...
int bst_iterator_has_next(struct bst_iterator* iter) {
  if ((stack_isempty(iter->s)) && (stack_isempty(iter->temp)))
    return(0);
  else
    return(1);
}
